...
parser$add_argument('--height', required=TRUE, help='Path to the Height summary statistics file')
parser$add_argument('--qc', required=TRUE, help='Path to the QCed SNPs file')
...
height <- fread(args$height) %>%
    # Change alleles to upper cases
    .[,c("A1","A2"):=list(toupper(A1), toupper(A2))]

# Read in QCed SNPs
qc <- fread(args$qc, header=FALSE)
...
fwrite(bim[,c("SNP", "B.A1")], args$output_a1, col.names=FALSE, sep="\t")
...
write.table(mismatch, args$output_mismatch, quote=FALSE, row.names=FALSE, col.names=FALSE)
